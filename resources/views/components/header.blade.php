    ...
    <div class="flex h-12 items-center justify-between">
      ...
      <div class="hidden md:flex md:items-center md:justify-center w-full">
        ...
        @foreach($categories as $category)
          <x-nav-link-header-news type="default" href="/category/{{ $category->slug }}" :active="request()->is('categories/' . $category->slug)">{{ $category->name }}</x-nav-link-header-news>
        @endforeach
      ...
      <div class="md:hidden md:items-center md:justify-center w-full">
        <div class="px-4 pt-2">
          ...
          @foreach($categories as $category)
          <x-nav-link-header-news type="default" href="/category/{{ $category->slug }}" :active="request()->is('categories/' . $category->slug)" mobile="true">{{ $category->name }}</x-nav-link-header-news>
        @endforeach
